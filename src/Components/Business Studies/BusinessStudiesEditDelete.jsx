...
const BusinessStudiesEditDelete = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(`${BASE_URL}course-business-get`);
      const data = await response.json();
      setBusinessStudiesData(data);
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    const url = editId
      ? `${BASE_URL}course-business-update/${editId}`
      : `${BASE_URL}course-business`;
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`${BASE_URL}course-business-delete/${id}`, {
        method: "DELETE",
      });
  ...
  return (
    <div className="max-w-4xl mx-auto mt-10 bg-white p-6 shadow rounded-xl">
      {/* TABLE */}
      <h3 className="text-xl font-semibold mb-4">Business Studies Data</h3>
...
export default BusinessStudiesEditDelete;
