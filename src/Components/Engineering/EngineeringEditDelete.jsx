...
const EngineeringEditDelete = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(`${BASE_URL}course-engineering-get`);
  ...
  const handleSubmit = async (e) => {
    ...
    const url = editId
      ? `${BASE_URL}course-engineering-update/${editId}`
      : `${BASE_URL}course-engineering`;
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(
        `${BASE_URL}course-engineering-delete/${id}`,
        {
          method: "DELETE",
        }
      );
  ...
  return (
    <div className="max-w-4xl mx-auto mt-10 bg-white p-6 shadow rounded-xl">
      {/* TABLE */}
      <h3 className="text-xl font-semibold mb-4">Engineering Data</h3>
...
export default EngineeringEditDelete;
