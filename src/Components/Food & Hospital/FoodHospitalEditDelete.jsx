...
const FoodHospitalEditDelete = () => {
  ...
  const [FoodHospitalData, setFoodHospitalData] = useState([]);
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(`${BASE_URL}course-food-hospitality-get`);
      const data = await response.json();
      setFoodHospitalData(data);
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    const url = editId
      ? `${BASE_URL}course-food-hospitality-update/${editId}`
      : `${BASE_URL}course-food-hospitality`;
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(
        `${BASE_URL}course-food-hospitality-delete/${id}`,
        {
          method: "DELETE",
        }
      );
  ...
  return (
    <div className="max-w-4xl mx-auto mt-10 bg-white p-6 shadow rounded-xl">
      {/* TABLE */}
      <h3 className="text-xl font-semibold mb-4">Food and Hospital Data</h3>
...
export default FoodHospitalEditDelete;
