...
const NursingEditDelete = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(`${BASE_URL}course-nurse-get`);
  ...
  const handleSubmit = async (e) => {
    ...
    const url = editId
      ? `${BASE_URL}course-nurse-update/${editId}`
      : `${BASE_URL}course-nurse`;
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`${BASE_URL}course-nurse-delete/${id}`, {
        method: "DELETE",
      });
  ...
  return (
    <div className="max-w-4xl mx-auto mt-10 bg-white p-6 shadow rounded-xl">
      {/* TABLE */}
      <h3 className="text-xl font-semibold mb-4">Nursing Data</h3>
...
export default NursingEditDelete;
