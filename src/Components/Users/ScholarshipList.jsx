...
const ScholarshipList = () => {
  ...
  // ✅ Fetch from /scholarship-register
  ...
  // ✅ Delete from /scholarship-register/:id
  const handleDelete = async (id) => {
    ...
    if (confirm.isConfirmed) {
      try {
        const response = await fetch(`${BASE_URL}scholarship-register/${id}`, {
          method: "DELETE",
        });
          ...
          {currentItems.length === 0 ? (
            ...
            currentItems.map((user) => (
              <tr key={user.id} className="odd:bg-white even:bg-gray-50 hover:bg-gray-100">
                ...
                <td className="border border-gray-300 px-4 py-2">{user.scholarship_country}</td>
